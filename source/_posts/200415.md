...
![2020041501](https://cdn.jsdelivr.net/gh/Anyway521/blogpic@main/image/image2020041501.gif)

通过Js实现简单的Loading加载特效。
<!-- more -->

## 添加html、js代码
- 找到`\themes\yilia\layout\layout.ejs` ,在`<div id="container" q-class="show:isCtnShow">`的<u>上一行</u>添加：
    ``` html
    ...
    ```
- 找到`themes\yilia\layout\_partial\head.ejs`,在`<head></head>`标签里添加代码：
    ``` js
    <!-- Js控制Loding特效-->
    <script>
      //监听加载状态改变
    ...
    <!-- loading结束 -->
    ```
    这里之所以用`readyState == "interactive"`,是因为`complete`意味着所有资源加载完毕，但诸如`gitalk`和各类插件偶尔会因为网络因素迟迟加载不完，
    但这些并不是主要的东西（最主要的是文章主体）,关于加载状态具体的可以去看：<https://developer.mozilla.org/zh-CN/docs/Web/API/Document/readyState>
    <b><p style="color:red">上面的代码注意一定要放在百度统计、谷歌统计引用的代码之前，如下图：</p></b>  
     
    ![20200421233500](https://cdn.jsdelivr.net/gh/Anyway521/blogpic@main/image/image20200421233500.png)


## 添加css样式
找到`\themes\yilia\source\main.0cf68a.css`,在文件的<u>开头</u>添加代码：
``` css
/* Loding特效样式 */
.chromeframe {
    margin: 0.2em 0;
